...
image_t process_scan(ifstream& jpegfile, qtable_t qtable, sof_t& sof, htable_t* htable, sos_t& sos) {
    ...
    if(para_flag)
    {
        int16_t image_gpu[n_MCUs*n_blocks*64];
        int16_t det_image_gpu[n_MCUs*n_blocks*64];
        for(int pMCU=0;pMCU<n_MCUs;pMCU++)
        {
           for (int pblock = 0; pblock < n_blocks; pblock++) {
                for (int pp = 0; pp < 64; pp++) {
                    image_gpu[pMCU*n_blocks*64+pblock*64+pp]=image[pMCU][pblock][pp];
                }
             }
        }
        idct_gpu(image_gpu,det_image_gpu,n_MCUs*n_blocks*64);
        for(int pMCU=0;pMCU<n_MCUs;pMCU++)
        {
           for (int pblock = 0; pblock < n_blocks; pblock++) {
                for (int pp = 0; pp < 64; pp++) {
                    image[pMCU][pblock][pp]=det_image_gpu[pMCU*n_blocks*64+pblock*64+pp];
                }
             }
        }
    }
    ...
    {
        int16_t par_ycbcr[sof.Y*sof.X*channels];
        int16_t par_rgb[sof.Y*sof.X*channels];
        for(int pc=0;pc<channels;pc++)
        {
           for (int py = 0; py < sof.Y; py++) {
                for (int px = 0; px < sof.X; px++) {
                    par_ycbcr[pc*sof.X*sof.Y+py*sof.X+px]=ycbcr[pc][py][px];
                }
             }
        }
        to_rgb_gpu(par_ycbcr,par_rgb,sof.Y,sof.X,channels);
        for(int pc=0;pc<channels;pc++)
        {
           for (int py = 0; py < sof.Y; py++) {
                for (int px = 0; px < sof.X; px++) {
                    rgb[pc][py][px]=par_rgb[pc*sof.X*sof.Y+py*sof.X+px];
                }
             }
        }
    }
...
int main(int argc, char* argv[]) {
    cout<<"请输入图片路径（-debug进行调试，-par并行解码）："<<endl;    
    /* parse filename */
    ...
    if(filename.size()<=0)
    {
        cout<<"[Error] path error"<<endl;
        return -1;
    }
    startTime =clock();
    for(vector<string>::iterator file_arr=filename.begin();file_arr!=filename.end();file_arr++)
    {
        /* open jpeg */
        ifstream jpegfile;
        jpegfile.open(*file_arr, ifstream::binary);
        if (!jpegfile.is_open()) {
            cout << "[Error] fail to open file: " << *file_arr << endl;
            return -2;
        }
